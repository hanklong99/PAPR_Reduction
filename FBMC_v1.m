...
for k_EbN0 = 1:length(EbN0_dB)
    ...
    % Modulator
        for nframe=1:Frame
            ...
            graycode(index_2) = 3;
            % mapped QAM data
            data_QAM(:,nframe)= sqrt(2)*exp(1j * graycode* pi/2 + 1j * pi/4);    
    ...
    % Plot Spectrum
        %stem(abs(y));
        yfft = [zeros(1,length(Tx)/2) fft(Tx) zeros(1,length(Tx)/2)];
        yTx = ifft(yfft);
        yTxFreq = fft(yTx,8192);
        yTxFreqAbs = abs(yTxFreq);
        yTxFreqAbs = yTxFreqAbs/max(yTxFreqAbs);
        yTxFreqAbsPwr = 20*log(yTxFreqAbs);
        plot(yTxFreqAbsPwr); xlim([0 length(yTxFreqAbsPwr)]);ylim([-100 0]);set(gca,'yticklabel',[]);    
        

        AWGN_noise=sqrt(N0(k_EbN0))*(randn(1,length(Tx))+1j*randn(1,length(Tx)));
        Tx=Tx*gain(m)+AWGN_noise;
    ...
    % Demodulator
        ...
        for nframe_oqam=1:OQAM_Frame
            r=Tx(1+(nframe_oqam-1)*N/2:(nframe_oqam-1)*N/2+K*N).*h; % We apply the filter
...
%         complex_noise=sqrt(N0(k_EbN0))*(randn(N,Frame)+1j*randn(N,Frame))/sqrt(2);
%         out=out+complex_noise;
