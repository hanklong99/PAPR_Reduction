%UnipolarNRZ_AWGN.m Simulate the performance of Unipolar NRZ in AWGN
%channels
% clear all;
% close all;
%Eb/N0 in dB
...
%Calculate the numerical BERs for different Eb/N0's. Each block has N_bit
%bits.
Num_BER = errs./(N_bit*block_count);
%Calculate the analytical BERs for different Eb/N0's
Ana_BER=Random_Q(sqrt(Eb_N0));
% figure;
semilogy(Eb_N0_dB, Num_BER, 'g-+');
hold on;
semilogy(Eb_N0_dB, Ana_BER, 'b-o');
grid on;
legend('Bipolar Numerical BER','Bipolar Analytical BER', 'Unipolar Numerical BER', 'Unipolar Analytical BER');
