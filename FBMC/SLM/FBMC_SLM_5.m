...
%% SLM Parameters
    N_SLM = 5; % number of sets of SLM Phase
    ...
    bers_SLM_5 = zeros(1,length(EbN0_dB));
    ber_count=0;
...
% FBMC modulation
            for nframe_oqam=1:OQAM_Frame
                ...
                x_SLM_5=ifft(OQAM(:,nframe_oqam));

                % Duplication of the signal
                x4=[x_SLM_5.' x_SLM_5.' x_SLM_5.' x_SLM_5.'];
...
figure(1);
semilogy(0:EbN0_dB(end),bers_SLM_5,'p-');
xlabel('E_b/N_0 [dB]'), ylabel('BER'); axis([0 15 1e-7 1]);
grid on;

figure(2);
papr_db=10*log10(Peak./Average);
[f,x_SLM_5]=ecdf(papr_db);
ccdf_SLM_5=1-f;
marker_idx = 1:1e3:length(x_SLM_5);
semilogy(x_SLM_5,ccdf_SLM_5,'-s','MarkerIndices',marker_idx);grid on; hold on;
