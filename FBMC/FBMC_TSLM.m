...
% TSLM
    ...
    Inv_D= inv(D);
    %% SLM Parameters
    T_TSLM=zeros(N_TSLM,K*N+(Frame*2-1)*N/2+N/2);
...
%% Calculate Aeverage Power
    ...
    % Modulator
        ...
        % Phase rotation
        for i=1:N_TSLM
            T_TSLM(i,:)=ifft(gray_P(:,i))*sqrt(K*N+(Frame*2-1)*N/2+N/2);
...
%% Rayleight Fading Multipath: Filter
        ...
        Rx_TSLM = Tx_ray(1:K*N+(Frame*2-1)*N/2+N/2)./T_TSLM(index_min,:);
...
%% Demodulator
        ...
        for nframe_oqam=1:OQAM_Frame
            r=Rx_TSLM(1+(nframe_oqam-1)*N/2:(nframe_oqam-1)*N/2+K*N).*h; % We apply the filter
