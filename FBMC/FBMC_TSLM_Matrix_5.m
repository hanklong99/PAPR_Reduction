%% Initialization for transmission
    ...
    bers_TSLM_5 = zeros(1,length(EbN0_dB));
    ber_count=0;
...
%% Modulator
        ...
        for nframe_oqam=1:OQAM_Frame
            ...
            x_TSLM_5=ifft(OQAM(:,nframe_oqam));

            % Duplication of the signal
            x4=[x_TSLM_5.' x_TSLM_5.' x_TSLM_5.' x_TSLM_5.'];
...
%% errors locations
    ...
    bers_TSLM_5(k_EbN0) = ber;
end

%% Plot

figure(3);
semilogy(0:EbN0_dB(end),bers_TSLM_5,'p-');
xlabel('E_b/N_0 [dB]'), ylabel('BER'); axis([0 15 1e-3 1]);
grid on;

figure(4);
papr_db=10*log10(Peak./Average);
[f,x_TSLM_5]=ecdf(papr_db);
ccdf_TSLM_5=1-f;
marker_idx = 1:1e3:length(x_TSLM_5);
semilogy(x_TSLM_5,ccdf_TSLM_5,'-s','MarkerIndices',marker_idx);grid on; hold on;
