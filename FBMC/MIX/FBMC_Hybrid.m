%% Initialization for transmission
    ...
    bers_Hybrid_fbmc = zeros(1,length(EbN0_dB));
    ber_count=0;
...
%% Modulator
        ...
        for nframe_oqam=1:OQAM_Frame
            ...
            x_Hybrid_fbmc=ifft(OQAM(:,nframe_oqam));

            % Duplication of the signal
            x4=[x_Hybrid_fbmc.' x_Hybrid_fbmc.' x_Hybrid_fbmc.' x_Hybrid_fbmc.'];
        ...
        % Companding
        mu_QPSK= (mu_A*log(1+mu*abs(Tx_TSLM)/mu_A)/log(1+mu)).*sign(Tx_TSLM);
        Tx=mu_QPSK;
        
        % Record PAPR information
...
%% Rayleight Fading Multipath: Filter
        ...
        Rx_DeCompd = (mu_A/mu)*(exp(abs(Rx_TSLM)*log(1+mu)/mu_A)-1).*sign(Rx_TSLM);
...
%% Demodulator
        ...
        for nframe_oqam=1:OQAM_Frame
            r=Rx_DeCompd(1+(nframe_oqam-1)*N/2:(nframe_oqam-1)*N/2+K*N,1).'.*h; % We apply the filter
...
%% errors locations
    ...
    bers_Hybrid_fbmc(k_EbN0) = ber;
end

%% Plot

figure(3);
semilogy(0:EbN0_dB(end),bers_Hybrid_fbmc,'p-');
xlabel('E_b/N_0 [dB]'), ylabel('BER'); axis([0 30 1e-4 1]);
grid on;

figure(4);
papr_db=10*log10(Peak./Average);
[f,x_Hybrid_fbmc]=ecdf(papr_db);
ccdf_Hybrid_fbmc=1-f;
marker_idx = 1:1e3:length(x_Hybrid_fbmc);
semilogy(x_Hybrid_fbmc,ccdf_Hybrid_fbmc,'-s','MarkerIndices',marker_idx);grid on; hold on;
