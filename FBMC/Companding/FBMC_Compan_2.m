...
%% Comapnding
    bers_Compd_2=zeros(size(EbN0_dB));
...
figure(1);
semilogy(0:EbN0_dB(end),bers_Compd_2, 'p-');grid on; hold on;% Companding Simulation
xlabel('E_b/N_0 [dB]'), ylabel('BER'); axis([0 15 1e-7 1]);


figure(2)
papr_db_Compd=10*log10(Peak_Compd./Average_Compd);
[f_Compd_2,x_Compd_2]=ecdf(papr_db_Compd);
ccdf_Compd_2=1-f_Compd_2;
marker_idx = 1:1e3:length(x_Compd_2);
semilogy(x_Compd_2,ccdf_Compd_2,'*-r','MarkerIndices',marker_idx);grid on; hold on;
