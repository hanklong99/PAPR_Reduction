...
for k_EbN0 = 1:length(EbN0_dB)
    ...
    for m = 1:N_iter
        ...
        Rx=y+complex_noise;
        %Calculate BER
        TSLM_Phase=zeros(length(data_QPSK), 1); % do not need SLM Phase
        ber_count_temp = OFDM_Con(TSLM_Phase,Rx, symbol, Ng, Nframe, Nfft, Nsym, symbol_total);
        ber_count=ber_count+ber_count_temp;
         ...
         index_min=a;
         Rx_TSLM = OFDM_TSLM(index_min, :) + complex_noise;
         
         ber_temp_TSLM = TSLM_Conv(T_TSLM(:,:,index_min),Rx_TSLM, symbol, Ng, Nframe, Nfft, Nsym, symbol_total);
         ber_count_TSLM = ber_count_TSLM+ber_temp_TSLM;
    ...
    ber_TSLM=ber_count_TSLM/symbol_total/N_iter/2;
    bers_TSLM(k_EbN0)= ber_TSLM;
end
...
semilogy(0:EbN0_dB(end),bers_TSLM, 'p-');% SLM Simulation

legend('AWGN Analytic', 'AWGN Simulation', 'TSLM Simulation');
