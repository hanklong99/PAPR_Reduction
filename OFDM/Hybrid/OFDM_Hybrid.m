...
bers_Hybrid=zeros(size(EbN0_dB));
    ...
    for m = 1:N_iter
        ...
        Rx_DeCompd = (mu_A/mu)*(exp(abs(Rx_whole)*log(1+mu)/mu_A)-1).*sign(Rx_whole);
         ...
         for k = 1:Nframe % N grames
             ...
             t_QPSK(kk1) = remove_GI(Ng,Nsym,Rx_DeCompd(kk2));
...
semilogy(0:EbN0_dB(end),bers_Hybrid, 'p-');% SLM Simulation
...
papr_db_Hybrid=10*log10(Peak./Average);
[f_Hybrid,x_Hybrid]=ecdf(papr_db_Hybrid);
ccdf_Hybrid=1-f_Hybrid;

marker_idx = 1:1e3:length(x_Hybrid);
semilogy(x_Hybrid,ccdf_Hybrid,'*-r','MarkerIndices',marker_idx);
